...
/**
 * Handle asynchronous cloud sensor logging requests via a Binder interface. Sensor event
 * s are
...
public class CloudPublisherService extends Service {
    ...
    private AtomicInteger runnableIndex = new AtomicInteger(1);
    ...
    private final Runnable mSensorConsumerRunnable = new Runnable() {
        ...
        public void run() {
            ...
            try {
                initializeIfNeeded();
                processCollectedSensorData(runnableIndex.get());
                mUnsuccessfulTentatives.set(0);
                runnableIndex.incrementAndGet();
            } catch (Throwable t) {
    ...
    @WorkerThread
    private void processCollectedSensorData(int index) {
