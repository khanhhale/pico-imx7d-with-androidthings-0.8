...
public class SensorHubActivity extends Activity {
        ...
        sensorHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Log.e(TAG, String.format(Locale.getDefault(), "Cannot collect sensor data, will try again in %d ms", SENSOR_READ_INTERVAL_MS), t);
                }
                Log.i(TAG, "Execute handler");
    ...
    private Bmx280 connectToBmx280() {
        try {
            Log.d(TAG, "BD default bmx: "+BoardDefaults.getI2cBusForSensors());
    ...
    private Button connectToMotionDetector() {
        try {
            Log.d(TAG, "BD default motion: "+BoardDefaults.getGPIOForMotionDetector());
        ...
        //if (mEnvironmentalSensor != null) {
            try {
                ...
                if (now >= INITIAL_VALID_TIMESTAMP) {
                    ...
                    output.add(new SensorData(now, SENSOR_TYPE_AMBIENT_PRESSURE_DETECTION, data[1]));
                } else {
                    Log.i(TAG, "Ignoring sensor readings because timestamp is invalid. " + "Please, set the device's date/time");
                }
            } catch (Throwable t) {
