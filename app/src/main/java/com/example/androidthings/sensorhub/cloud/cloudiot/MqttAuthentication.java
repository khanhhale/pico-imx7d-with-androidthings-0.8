...
public class MqttAuthentication {
  ...
  public void exportPublicKey(File destination) throws IOException, GeneralSecurityException {
    StringBuilder exportKey = new StringBuilder();
    exportKey.append((new String(getCertificatePEM().getBytes(), "UTF-8")).toString());
    System.out.println(exportKey.toString());

    //Log.i(TAG, "Begin Public Key\n");
    //Log.i(TAG, (new String(getCertificatePEM().getBytes(), "UTF-8")).toString());
    //Log.i(TAG, "End Public Key\n");
